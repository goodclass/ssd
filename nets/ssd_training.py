...
class MultiboxLoss(nn.Module):
    ...
    def forward(self, y_true, y_pred):
        ...
        #   分类的loss  batch_size,8732,21 -> batch_size,8732
        conf_loss = self._softmax_loss(y_true[:, :, 4:-1], y_pred[:, :, 4:])
        #   框的位置的loss  batch_size,8732,4 -> batch_size,8732
        ...
        #   每一张图的正样本的个数 num_pos     [batch_size,]
        num_pos = torch.sum(y_true[:, :, -1], axis=-1)

        #   每一张图的负样本的个数 num_neg     [batch_size,]
        ...
        #   batch_size,8732  把不是背景的概率求和，求和后的概率越大 代表越难分类。
